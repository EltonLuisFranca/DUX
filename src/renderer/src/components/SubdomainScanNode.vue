<template>
  ...
  >
    ...
    <div class="ss-header" :style="{ background: data.headerColor || undefined }">
      <span class="status-dot" :class="statusDotClass" />
      <span class="ss-title">{{ data.name }}</span>
    ...
    <div class="tab-body nodrag nowheel nopan">
      <div v-if="activeTab === 'target'" class="pane">
        <div class="field-block">
          ...
          <input
            ...
            @input="syncData"
          />
          <p class="hint">Recon inicial de superfície de ataque — testa subdomínio.dominio via resolução DNS.</p>
        </div>

        <div class="field-row">
          <label class="exec-field">
            Timeout por tentativa (ms)
            <input v-model.number="timeoutMs" type="number" min="200" max="15000" :disabled="running" @input="syncData" />
          </label>
        </div>

        <label class="checkbox-label">
          <input type="checkbox" v-model="checkHttp" @change="syncData" />
          Verificar status HTTP dos subdomínios encontrados
        </label>
        <p class="hint">Além da resolução DNS, tenta um HEAD (https, depois http) e também serve de fallback quando o DNS não resolve.</p>
      </div>
      ...
      <div v-else-if="activeTab === 'wordlist'" class="pane">
        ...
        <div v-if="wordlistMode === 'common'">
          <p class="hint">Subdomínios mais comuns pra um primeiro reconhecimento (infra, ambientes, serviços internos).</p>
      ...
      <div v-else class="pane">
        ...
        <div v-if="!running && !showConfirm" class="start-row">
          ...
          <span v-else-if="wordlistCount === 0" class="hint">Nenhum subdomínio configurado na aba Wordlist.</span>
        </div>

        <div v-if="showConfirm" class="confirm-panel">
          <p>
            Confirma a varredura contra <strong>{{ domain }}</strong> — {{ wordlistCount }} subdomínios?
          </p>
        ...
        <div v-if="running" class="progress-panel">
          ...
          <div class="progress-row">
            <span class="hint">{{ progressCount }}/{{ progressTotal }} subdomínios</span>
            <button class="btn-secondary" @click="stopScan">Parar</button>
          </div>
        </div>

        <div v-if="liveFound.length" class="banner warn">
          <strong>{{ liveFound.length }} subdomínio(s) encontrado(s):</strong>
          <div v-for="f in liveFound" :key="f.hostname" class="finding-row">
            {{ f.hostname }}
            <span class="finding-status">
              {{ f.ips?.length ? f.ips.join(', ') : '' }}{{ f.httpStatus ? ' · HTTP ' + f.httpStatus : '' }}
            </span>
          </div>
        </div>
        <div v-else-if="lastResult && !running && !lastResult.error" class="banner ok">
          Nenhum subdomínio encontrado em {{ lastResult.totalSubdomains }} testados.
        </div>

        <div v-if="lastResult && !lastResult.error" class="summary-row">
          <span class="hint">
            {{ lastResult.cancelled ? 'Parado manualmente' : 'Concluído' }} — {{ lastResult.totalSubdomains }} subdomínios em
        ...
        <div v-if="showLogModal" class="logs-modal-backdrop" @mousedown.self="showLogModal = false">
          <div class="logs-modal nodrag nowheel nopan">
            ...
            <div class="logs-modal-toolbar">
              <input v-model="logSearch" type="text" class="logs-search" placeholder="Filtrar subdomínio..." />
            ...
            <div class="logs-modal-body">
              <div v-if="!filteredAttempts.length" class="logs-empty">Nenhum subdomínio verificado ainda.</div>
              <div
              ...
              >
                ...
                <span class="log-cred">{{ a.hostname }}</span>
                <span class="log-status">{{ a.found ? 'achado' : 'não achou' }}</span>
                <span v-if="a.found" class="log-tag success">
                  {{ a.ips?.length ? a.ips.join(', ') : a.method }}{{ a.httpStatus ? ' · HTTP ' + a.httpStatus : '' }}
                </span>
              </div>
            </div>
          </div>
...
import { runSubdomainScan } from '../lib/bridgeClient'

// prévia client-side da wordlist "common" do bridge (bridge/subdomainScan.js)
// — mesmo motivo do TOP20_PORTS_CLIENT em PortScanNode.vue: mostrar a lista
// na aba sem ida e volta ao bridge.
...
const timeoutMs = ref(props.data.timeoutMs ?? 3000)
const checkHttp = ref(props.data.checkHttp ?? true)

const wordlistMode = ref(props.data.wordlistMode || 'common')
const customWordlist = ref(props.data.customWordlist || '')

const concurrency = ref(props.data.concurrency ?? 30)
...
const wordlistCount = computed(() => {
  ...
  return COMMON_SUBDOMAINS_CLIENT.length
})

const canStart = computed(() => Boolean(domain.value.trim()) && wordlistCount.value > 0 && !running.value)
...
const filteredAttempts = computed(() => {
  ...
  if (term) list = list.filter((a) => a.hostname.toLowerCase().includes(term))
...
async function copyLog() {
  const text = filteredAttempts.value
    .map((a) => `#${a.seq} ${a.hostname} ${a.found ? 'achado (' + (a.ips?.join(', ') || a.method) + (a.httpStatus ? ' · HTTP ' + a.httpStatus : '') + ')' : 'não achou'}`)
    .join('\n')
  try {
    ...
    console.error('[subdomain-scan-node] copy failed', err)
  }
}
