<template>
  <div class="create-form">
    ...
    <label class="field-label" for="new-merlin-host">Endereço do Ollama</label>
    <input
      id="new-merlin-host"
      v-model="host"
      class="field-input"
    ...
    <label class="field-label" for="new-merlin-token">Token de autenticação (opcional)</label>
    <input
      id="new-merlin-token"
      v-model="token"
      class="field-input"
    ...
    <label class="field-label" for="new-merlin-model">Modelo</label>
    <select id="new-merlin-model" v-model="selectedModel" class="field-input" :disabled="status !== 'ready'">
...
// Se o usuário tiver esse modelo instalado, pré-seleciona ele — foi o
// escolhido como ponto de partida pra tool-calling/agentic use (ver conversa
// sobre qwen3:8b vs dolphin3:8b). Cai pro primeiro modelo da lista senão.
...
async function fetchModels() {
  ...
  try {
    ...
    models.value = list
    selectedModel.value = list.find((m) => m === PREFERRED_MODEL) || list[0] || ''
    status.value = 'ready'
  } catch (err) {
    console.error('[merlin] failed to list models', err)
    status.value = 'error'
  }
}
...
onMounted(() => {
  fetchModels()
  nextTick(() => document.getElementById('new-merlin-host')?.focus())
})
</script>
